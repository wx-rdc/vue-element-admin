...
export default {
  ...
  methods: {
    ...
    backToTop() {
      let start = document.querySelector(".app-main").scrollTop;
      let i = 0;
      this.interval = setInterval(() => {
        const next = Math.floor(this.easeInOutQuad(10 * i, start, -start, 500));
        if (next <= this.backPosition) {
          document.querySelector(".app-main").scrollTo(0, this.backPosition);
          clearInterval(this.interval);
        } else {
          document.querySelector(".app-main").scrollTo(0, next);
        }
        i++;
      }, 16.7);
    },
